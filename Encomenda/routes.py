...
encomenda_blueprint = Blueprint('encomenda_api_routes', __name__, url_prefix='/api/encomendas')
...
def get_Utilizador(api_key):
    ...
    if response.status_code != 200:
        return {'message':'Nao autorizado 2.'}

    utilizador = response.json()
    return utilizador

@encomenda_blueprint.route('/', methods = ['GET'])
def get_Encomendas_Pendente():
    api_key = request.headers.get('Authorization')

    if not api_key:
        return jsonify({'message':'Nao esta autenticado'}),401
    response = get_Utilizador(api_key)
    utilizador = response.get('result')
    if not utilizador:
        return jsonify({'message':'Nao esta autenticado'}),401

    encomendaPendente = Encomenda.query.filter_by(utilizador_id=utilizador['id'], aberta =1).first()

    if encomendaPendente:
        return jsonify({'result':encomendaPendente.serializar()}), 200
    else:
        return jsonify({'message':'Sem encomanda pendente'}),401
...
def adicionar_item_encomenda():
        ...
        if not encomendaPendente:
            ...
            if not encomendaPendente:
                ...
                linhaEncomenda = EncomendaLinha(artigoId = artigoId, quantidade =quantidade)
                encomendaPendente.linhas_em_encomenda.append(linhaEncomenda)
            else:
                encontrou = False
                for linha in encomendaPendente.linhas_em_encomenda:
...
def checkout():
        ...
        if encomendaPendente:
            ...
            return jsonify({'result': 'sem encomenda pendente'})